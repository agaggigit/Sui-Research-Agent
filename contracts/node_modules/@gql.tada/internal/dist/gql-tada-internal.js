  ...
  return n => r => {
    ...
    r(start((e => {
      if (1 === e) {
        ...
        for (var d = 0, l = a, u = a.length; d < u; d++) {
          l[d](0);
        }
      }
    })));
    ...
    if (1 === n.length) {
      e((e => {
        if (0 === e) {
          ...
          a = !1;
          for (var s = 0, d = n, l = n.length; s < l; s++) {
            d[s](e);
          }
        }
      }));
...
var stringify = (e, n) => {
  if (null === e || f.has(e)) {
    ...
    return r + "]";
  } else if (!n && (v !== NoopConstructor && e instanceof v || m !== NoopConstructor && e instanceof m)) {
    return "null";
  }
...
var extract = (e, n, r) => {
  if (null == r || "object" != typeof r || r.toJSON || f.has(r)) {} else if (Array.isArray(r)) {
    for (var a = 0, t = r.length; a < t; a++) {
      extract(e, `${n}.${a}`, r[a]);
    }
  } else if (r instanceof v || r instanceof m) {
    e.set(n, r);
  } else {
    f.add(r);
...
var sanitizeDocument = e => e.split(h).map(replaceOutsideStrings).join("").trim();
...
var stringifyDocument = e => {
  ...
  } else if (e.loc && k.get(e.__key) === e) {
    n = e.loc.source.body;
  } else {
    n = E.get(e) || sanitizeDocument(a.print(e));
    E.set(e, n);
  }
...
var mergeResultPatch = (e, n, r, a) => {
  ...
  if (s) {
    var _loop = function(e) {
      ...
      for (var u = 0, c = s.length; u < c; n = s[u++]) {
      ...
      if (e.items) {
        ...
        for (var p = 0, v = e.items.length; p < v; p++) {
          r[f + p] = deepMerge(r[f + p], e.items[p]);
        }
...
var makeFetchOptions = (e, n) => {
  ...
  var i = ((e, n) => {
    if (n && ("query" !== e.kind || !e.context.preferGetMethod)) {
      ...
      var a = (e => {
        var n = new Map;
        if (v !== NoopConstructor || m !== NoopConstructor) {
          f.clear();
...
var g = "undefined" != typeof TextDecoder ? new TextDecoder : null;

var N = /boundary="?([^=";]+)"?/i;

var T = /data: ?([^\n]+)/;

var toString = e => "Buffer" === e.constructor.name ? e.toString() : g.decode(e);
  ...
  return onEnd((() => {
    ...
    try {
      ...
      for await (var l of s) {
        ...
        t = t ? mergeResultPatch(t, l, i, d) : makeResult(e, l, i);
        a = !1;
        yield t;
        a = !0;
      }
...
var w = function Client(e) {
  ...
  var makeResultSource = e => {
    var n = takeUntil(filter((n => "teardown" === n.kind && n.key === e.key))(d.source))(filter((n => n.operation.kind === e.kind && n.operation.key === e.key && (!n.operation.context._instance || n.operation.context._instance === e.context._instance)))(h));
    if ("query" !== e.kind) {
  ...
  var m = (e => ({client: n, forward: r, dispatchDebug: a}) => e.reduceRight(((e, r) => {
    var t = !1;
    return r({
      client: n,
      forward(n) {
...
var getPeerSupportedFeatures = () => {
  if (!b) {
    ...
    return b = n.data ? toSupportedFeatures(n.data) : O;
  }
  return b;
};
...
var makeIntrospectionQuery = e => {
  if (I && A === e) {
    ...
    return I = _makeIntrospectionQuery(A = e);
  }
};
...
function loadFromSDL(a) {
  ...
  return {
    ...
    load: async e => e || !d ? d = await load() : d,
    notifyOnUpdate(e, r) {
      if (!i.size) {
        (async () => {
          if (n.sys.watchFile) {
            var e = n.sys.watchFile(a.file, reload, 250, {
            ...
            s = () => r.abort();
            var i = t.watch(a.file, {
              signal: r.signal,
              persistent: !1
            });
            try {
              for await (var o of i) {
                await reload();
              }
            } catch (e) {
...
function loadFromURL(e) {
  ...
  var introspect = async o => {
    var l = makeIntrospectionQuery(o);
    var u = await d.query(l, {});
    try {
      if (u.error) {
        throw u.error;
      } else if (u.data) {
  ...
  var load = async () => {
    if (!o) {
      ...
      var n = await d.query(e, {});
      if (n.error && n.error.graphQLErrors.length > 0) {
        try {
          var {introspection: r} = await introspect(O);
          o = introspectionToSupportedFeatures(r);
        } catch (e) {
          o = O;
        }
      } else if (n.data && !n.error) {
        o = toSupportedFeatures(n.data);
      } else if (n.error) {
        o = null;
        throw n.error;
      } else {
...
var K = [ "name", "tadaOutputLocation", "tadaTurboLocation", "tadaPersistedLocation" ];
...
var S = [ "tadaOutputLocation", "tadaTurboLocation", "tadaPersistedLocation" ];

var L = new Set([ "tadaOutputLocation" ]);

var schemaIdentity = (e, n) => {
  if (!n) {
    return "";
  }
  if ("object" == typeof n) {
    return `url:${n.url}\0${JSON.stringify(n.headers || null)}`;
  }
  if (getURLConfig(n)) {
    return `url:${n}`;
  }
...
var _ = "undefined" != typeof require ? require.resolve.bind(require) : i.createRequire("undefined" == typeof document ? require("url").pathToFileURL(__filename).href : o && o.src || new URL("dist/gql-tada-internal.js", document.baseURI).href).resolve;
...
var toTSConfigPath = e => ".json" !== s.extname(e) ? s.resolve(l, e, x) : s.resolve(l, e);
...
var resolvePluginEntry = async (e, n) => {
  var r = (e => e && e.compilerOptions && e.compilerOptions.plugins && e.compilerOptions.plugins.find((e => "@0no-co/graphqlsp" === e.name || "gql.tada/lsp" === e.name || "gql.tada/ts-plugin" === e.name)) || null)(n);
  if (r) {
    return {
      pluginConfig: r,
      configPath: e
  ...
  var a = Array.isArray(n.extends) ? n.extends : n.extends ? [ n.extends ] : [];
  for (var t of a) {
    if (".json" !== s.extname(t)) {
      t += ".json";
    }
    try {
      var i = await resolveExtend(t, s.dirname(e));
      if (!i) {
        continue;
      }
      var o = await readTSConfigFile(i);
      var d = await resolvePluginEntry(i, o);
      if (d) {
        return d;
      }
    } catch (e) {}
...
var loadConfigs = async e => {
  ...
  var visit = async (e, n) => {
    ...
    var l;
    try {
      l = await readTSConfigFile(e);
    } catch (e) {
      if (n || !(e => "object" == typeof e && null !== e && "code" in e && ("ENOENT" === e.code || "ENOTDIR" === e.code))(e)) {
        throw e;
      }
      return;
    }
    var u = await resolvePluginEntry(e, l);
    if (u) {
      var c = {
        pluginConfig: u.pluginConfig,
        configPath: u.configPath,
        tsconfigPath: e,
        rootPath: s.dirname(e)
      };
      if (isSolutionStyleConfig(l)) {
        if (!i) {
          i = c;
        }
      } else {
        var f = `${toProjectKey(u.configPath)}\0${toProjectKey(c.rootPath)}`;
        if (!a.has(f)) {
          a.add(f);
          t.push(c);
        }
    ...
    if ((!u || isSolutionStyleConfig(l)) && Array.isArray(l.references)) {
      o = !0;
      for (var p of l.references) {
        if (!p || "string" != typeof p.path) {
          continue;
        }
        await visit((v = p.path, m = s.dirname(e), h = void 0, h = s.resolve(m, v), ".json" === s.extname(h) ? h : s.join(h, x)), !1);
      }
    }
    var v, m, h;
  };
  ...
  if (!t.length) {
    throw new TadaError(`Could not find a valid GraphQLSP plugin entry in: ${maybeRelative(n)}` + (o ? " or any of its referenced projects" : ""));
  }
  return t;
};
...
exports.extractIntrospectionHeader = function extractIntrospectionHeader(e) {
  var n = e.split("\n");
  var r = [];
  var a = !1;
  for (var t of n) {
...
exports.loadRef = function loadRef(e) {
  ...
  var a = {
    ...
    async load(e) {
      var n = getLoaders(e);
      await Promise.all(n.map((async ({input: n, loader: r}) => {
        var t = await r.load(e.reload);
        a.version++;
        if (n.name) {
          a.multi[n.name] = {
            ...n,
...
exports.outputIntrospectionFile = function outputIntrospectionFile(e, n) {
  ...
  if (/\.d\.ts$/.test(n.fileType)) {
    ...
    if ("string" != typeof e && n.shouldPreprocess) {
      a.push(`export type ${C} = ${preprocessIntrospectionTypes(e)};\n`, M, `export type introspection = ${preprocessIntrospection(e, C)};\n`, "import * as gqlTada from 'gql.tada';\n");
    } else {
      a.push(M, `export type introspection = ${stringifyJson(e)};\n`, "import * as gqlTada from 'gql.tada';\n");
    }
    if (!("name" in e) || !e.name) {
    ...
    return [ r, F, `const introspection = ${t} as const;\n`, "export { introspection };" ].join("\n");
  }
...
exports.validateUniqueOutputLocations = e => {
  for (var n of S) {
    var r = new Map;
    for (var a of e) {
      var t = a.label || a.projectPath;
      var i = "schemas" in a.config ? a.config.schemas : [ a.config ];
      for (var o of i) {
        var d = o[n];
        if (!d) {
          continue;
        }
        var l = s.resolve(a.projectPath, d);
        if ("win32" === process.platform) {
          l = l.toLowerCase();
        }
        var u = schemaIdentity(a.projectPath, o.schema);
        var c = r.get(l);
        if (c && c.label !== t) {
          if (!(L.has(n) && c.schema === u)) {
            throw new TadaError(`Projects '${c.label}' and '${t}' resolve '${n}' to the same file: ${d}`);
          }
        } else if (!c) {
          r.set(l, {
            label: t,
