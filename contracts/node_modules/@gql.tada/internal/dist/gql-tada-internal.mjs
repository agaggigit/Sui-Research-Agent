...
import { GraphQLSchema as n, GraphQLObjectType as t, GraphQLID as i, executeSync as o, buildClientSchema as s, buildSchema as l } from "graphql";

import { Kind as u, GraphQLError as c, parse as d, print as f, OperationTypeNode as p } from "@0no-co/graphql.web";

import * as v from "node:fs/promises";
...
var maybeRelative = r => {
  var a = e.relative(y, r);
  return !a.startsWith("..") ? a : r;
};

var toError = e => e instanceof Error ? e : new Error(`${e}`);
  ...
  return r => a => {
    ...
    a(start((e => {
      if (1 === e) {
        ...
        for (var l = 0, u = n, c = n.length; l < c; l++) {
          u[l](0);
        }
      }
    })));
...
var stringify = (e, r) => {
  if (null === e || g.has(e)) {
    ...
    return a + "]";
  } else if (!r && (T !== NoopConstructor && e instanceof T || w !== NoopConstructor && e instanceof w)) {
    return "null";
  }
...
var extract = (e, r, a) => {
  if (null == a || "object" != typeof a || a.toJSON || g.has(a)) {} else if (Array.isArray(a)) {
    for (var n = 0, t = a.length; n < t; n++) {
      extract(e, `${r}.${n}`, a[n]);
    }
  } else if (a instanceof T || a instanceof w) {
    e.set(r, a);
  } else {
    g.add(a);
...
var T = "undefined" != typeof File ? File : NoopConstructor;

var w = "undefined" != typeof Blob ? Blob : NoopConstructor;

var O = /("{3}[\s\S]*"{3}|"(?:\\.|[^"])*")/g;

var I = /(?:#[^\n\r]+)?(?:[\r\n]+|$)/g;

var replaceOutsideStrings = (e, r) => r % 2 == 0 ? e.replace(I, "\n") : e;

var sanitizeDocument = e => e.split(O).map(replaceOutsideStrings).join("").trim();
...
var stringifyDocument = e => {
  ...
  } else if (e.loc && b.get(e.__key) === e) {
    r = e.loc.source.body;
  } else {
    r = A.get(e) || sanitizeDocument(f(e));
    A.set(e, r);
  }
...
var getOperationName = e => {
  for (var r of e.definitions) {
    if (r.kind === u.OPERATION_DEFINITION) {
...
var makeFetchOptions = (e, r) => {
  ...
  var i = ((e, r) => {
    if (r && ("query" !== e.kind || !e.context.preferGetMethod)) {
      ...
      var n = (e => {
        var r = new Map;
        if (T !== NoopConstructor || w !== NoopConstructor) {
          g.clear();
...
var toString = e => "Buffer" === e.constructor.name ? e.toString() : L.decode(e);
  ...
  return onEnd((() => {
    ...
    try {
      ...
      for await (var u of s) {
        ...
        t = t ? mergeResultPatch(t, u, i, l) : makeResult(e, u, i);
        n = !1;
        yield t;
        n = !0;
      }
...
var _ = function Client(e) {
  ...
  var makeResultSource = e => {
    var r = takeUntil(filter((r => "teardown" === r.kind && r.key === e.key))(s.source))(filter((r => r.operation.kind === e.kind && r.operation.key === e.key && (!r.operation.context._instance || r.operation.context._instance === e.context._instance)))(h));
    if ("query" !== e.kind) {
  ...
  var f = noop;
  if ("production" !== process.env.NODE_ENV) {
    var {next: p, source: v} = makeSubject();
    d.subscribeToDebugTarget = e => subscribe(e)(v);
    f = p;
  }
  var m = (e => ({client: r, forward: a, dispatchDebug: n}) => e.reduceRight(((e, a) => {
    var t = !1;
    return a({
      client: r,
      forward(r) {
...
var getPeerSupportedFeatures = () => {
  if (!M) {
    ...
    return M = r.data ? toSupportedFeatures(r.data) : D;
  }
  return M;
};
...
var makeIntrospectionQuery = e => {
  if (F && C === e) {
    ...
    return F = _makeIntrospectionQuery(C = e);
  }
};
...
function loadFromSDL(e) {
  ...
  var load = async () => {
    ...
    if (".json" === a) {
      ...
      var u = makeIntrospectionQuery(getPeerSupportedFeatures());
      var c = o({
        schema: i,
        document: u
      });
  ...
  return {
    ...
    load: async e => e || !u ? u = await load() : u,
    notifyOnUpdate(r, o) {
      if (!n.size) {
        (async () => {
          if (a.sys.watchFile) {
            var r = a.sys.watchFile(e.file, reload, 250, {
            ...
            i = () => n.abort();
            var t = m.watch(e.file, {
              signal: n.signal,
              persistent: !1
            });
            try {
              for await (var o of t) {
                await reload();
              }
            } catch (e) {
...
var retryExchange = e => {
  ...
  return ({forward: e, dispatchDebug: s}) => l => {
    var {source: u, next: c} = makeSubject();
    var d = mergeMap((e => {
      ...
      r.delay = u;
      var d = filter((r => ("query" === r.kind || "teardown" === r.kind) && r.key === e.key))(l);
...
function loadFromURL(e) {
  ...
  var introspect = async i => {
    var u = makeIntrospectionQuery(i);
    var c = await l.query(u, {});
    try {
      if (c.error) {
        throw c.error;
      } else if (c.data) {
  ...
  var load = async () => {
    if (!i) {
      ...
      var r = await l.query(e, {});
      if (r.error && r.error.graphQLErrors.length > 0) {
        try {
          var {introspection: a} = await introspect(D);
          i = introspectionToSupportedFeatures(a);
        } catch (e) {
          i = D;
        }
      } else if (r.data && !r.error) {
        i = toSupportedFeatures(r.data);
      } else if (r.error) {
        i = null;
        throw r.error;
      } else {
...
function loadRef(e) {
  ...
  var n = {
    ...
    async load(e) {
      var r = getLoaders(e);
      await Promise.all(r.map((async ({input: r, loader: a}) => {
        var t = await a.load(e.reload);
        n.version++;
        if (r.name) {
          n.multi[r.name] = {
            ...r,
...
var P = [ "tadaOutputLocation", "tadaTurboLocation", "tadaPersistedLocation" ];

var $ = new Set([ "tadaOutputLocation" ]);

var schemaIdentity = (r, a) => {
  if (!a) {
    return "";
  }
  if ("object" == typeof a) {
    return `url:${a.url}\0${JSON.stringify(a.headers || null)}`;
  }
  if (getURLConfig(a)) {
    return `url:${a}`;
  }
...
var validateUniqueOutputLocations = r => {
  for (var a of P) {
    var n = new Map;
    for (var t of r) {
      var i = t.label || t.projectPath;
      var o = "schemas" in t.config ? t.config.schemas : [ t.config ];
      for (var s of o) {
        var l = s[a];
        if (!l) {
          continue;
        }
        var u = e.resolve(t.projectPath, l);
        if ("win32" === process.platform) {
          u = u.toLowerCase();
        }
        var c = schemaIdentity(t.projectPath, s.schema);
        var d = n.get(u);
        if (d && d.label !== i) {
          if (!($.has(a) && d.schema === c)) {
            throw new TadaError(`Projects '${d.label}' and '${i}' resolve '${a}' to the same file: ${l}`);
          }
        } else if (!d) {
          n.set(u, {
            label: i,
...
var stat = (e, r = isFile) => v.stat(e).then(r).catch((() => !1));

var j = "undefined" != typeof require ? require.resolve.bind(require) : h(import.meta.url).resolve;

var resolveExtend = async (e, r) => {
  try {
    return toTSConfigPath(j(e, {
      paths: [ r ]
    }));
  } catch (e) {
    return null;
...
var toTSConfigPath = r => ".json" !== e.extname(r) ? e.resolve(y, r, q) : e.resolve(y, r);

var readTSConfigFile = async e => {
  var r = toTSConfigPath(e);
  var n = await v.readFile(r, "utf8");
...
var toProjectKey = r => {
  var a = e.normalize(r);
  return "win32" === process.platform ? a.toLowerCase() : a;
};

var isSolutionStyleConfig = e => Array.isArray(e.references) && Array.isArray(e.files) && 0 === e.files.length && !e.include;

var resolvePluginEntry = async (r, a) => {
  var n = (e => e && e.compilerOptions && e.compilerOptions.plugins && e.compilerOptions.plugins.find((e => "@0no-co/graphqlsp" === e.name || "gql.tada/lsp" === e.name || "gql.tada/ts-plugin" === e.name)) || null)(a);
  if (n) {
    return {
      pluginConfig: n,
      configPath: r
  ...
  var t = Array.isArray(a.extends) ? a.extends : a.extends ? [ a.extends ] : [];
  for (var i of t) {
    if (".json" !== e.extname(i)) {
      i += ".json";
    }
    try {
      var o = await resolveExtend(i, e.dirname(r));
      if (!o) {
        continue;
      }
      var s = await readTSConfigFile(o);
      var l = await resolvePluginEntry(o, s);
      if (l) {
        return l;
      }
    } catch (e) {}
...
var loadConfigs = async r => {
  ...
  var visit = async (r, a) => {
    ...
    var u;
    try {
      u = await readTSConfigFile(r);
    } catch (e) {
      if (a || !(e => "object" == typeof e && null !== e && "code" in e && ("ENOENT" === e.code || "ENOTDIR" === e.code))(e)) {
        throw e;
      }
      return;
    }
    var c = await resolvePluginEntry(r, u);
    if (c) {
      var d = {
        pluginConfig: c.pluginConfig,
        configPath: c.configPath,
        tsconfigPath: r,
        rootPath: e.dirname(r)
      };
      if (isSolutionStyleConfig(u)) {
        if (!o) {
          o = d;
        }
      } else {
        var f = `${toProjectKey(c.configPath)}\0${toProjectKey(d.rootPath)}`;
        if (!t.has(f)) {
          t.add(f);
          i.push(d);
        }
    ...
    if ((!c || isSolutionStyleConfig(u)) && Array.isArray(u.references)) {
      s = !0;
      for (var p of u.references) {
        if (!p || "string" != typeof p.path) {
          continue;
        }
        await visit((v = p.path, m = e.dirname(r), h = void 0, h = e.resolve(m, v), ".json" === e.extname(h) ? h : e.join(h, q)), !1);
      }
    }
    var v, m, h;
  };
  ...
  if (!i.length) {
    throw new TadaError(`Could not find a valid GraphQLSP plugin entry in: ${maybeRelative(a)}` + (s ? " or any of its referenced projects" : ""));
  }
  return i;
};

var loadConfig = async e => (await loadConfigs(e))[0];
...
function extractIntrospectionHeader(e) {
  ...
  var o = a.join("\n");
  if (o.includes("An IntrospectionQuery representation")) {
    return null;
  }
  return o + "\n";
}
...
function outputIntrospectionFile(e, r) {
  ...
  if (/\.d\.ts$/.test(r.fileType)) {
    ...
    if ("string" != typeof e && r.shouldPreprocess) {
      n.push(`export type ${B} = ${preprocessIntrospectionTypes(e)};\n`, V, `export type introspection = ${preprocessIntrospection(e, B)};\n`, "import * as gqlTada from 'gql.tada';\n");
    } else {
      n.push(V, `export type introspection = ${stringifyJson(e)};\n`, "import * as gqlTada from 'gql.tada';\n");
    }
    if (!("name" in e) || !e.name) {
      n.push("declare module 'gql.tada' {", "  interface setupSchema {", "    introspection: introspection", "  }", "}");
    }
    return n.join("\n");
  } else if (/\.ts$/.test(r.fileType)) {
    var t = stringifyJson(e);
    return [ a, G, `const introspection = ${t} as const;\n`, "export { introspection };" ].join("\n");
  }
  throw new TadaError(`No available introspection format for "${r.fileType}" (expected ".ts" or ".d.ts")`);
}

export { TSError, TadaError, extractIntrospectionHeader, findTSConfigFile, getSchemaConfigForName, getSchemaNamesFromConfig, getURLConfig, load, loadConfig, loadConfigs, loadFromSDL, loadFromURL, loadRef, minifyIntrospectionQuery as minifyIntrospection, outputIntrospectionFile, parseConfig, preprocessIntrospection, readTSConfigFile, resolveTypeScriptRootDir, validateUniqueOutputLocations };
